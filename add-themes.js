...
// Tim vi tri truoc TOEIC_TESTS
const insertBefore = 'export const TOEIC_TESTS';
const idx = c.indexOf(insertBefore);
if (idx === -1) { console.log('Khong tim thay vi tri insert'); process.exit(1); }
...
// Chen truoc TOEIC_TESTS
// Tim vi tri ket thuc cua TOEIC_THEMES array (dau ];)
const themesEnd = c.lastIndexOf('];', idx);
if (themesEnd === -1) { console.log('Khong tim thay ket thuc array'); process.exit(1); }

const before = c.substring(0, themesEnd);
const after = c.substring(themesEnd);
const result = before + newThemes + after;

fs.writeFileSync('data/toeic.ts', result, 'utf8');

// Verify
const check = fs.readFileSync('data/toeic.ts', 'utf8');
console.log('technology found:', check.includes('technology'));
console.log('education found:', check.includes('education'));
